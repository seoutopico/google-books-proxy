...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (data[`ISBN:${isbnLimpio}`]) {
      publishedDate = data[`ISBN:${isbnLimpio}`].publish_date || null;
    }
    ...
    return res.status(200).json({
      isbn: isbnLimpio,
      fecha: publishedDate,
      fuente: publishedDate ? 'Open Library' : null,
      rawData: data // Incluir datos completos para depuración
    });
  } catch (error) {
    console.error('Error:', error);
    return res.status(500).json({ 
      error: 'Error al comunicarse con Open Library API',
